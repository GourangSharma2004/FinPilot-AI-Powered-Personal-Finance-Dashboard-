...
const fakeBackend = () => {
  ...
  mock.onPost('/post-login').reply(function (config) {
    console.log('Received login request:', config['data']);
    const user = JSON.parse(config['data']);
    console.log('Parsed user data:', user);
    
    const validUser = users.filter(usr => usr.email === user.username && usr.password === user.password);
    console.log('Valid user check result:', validUser);
      ...
      setTimeout(function () {
        if (validUser['length'] === 1) {
          ...
          const authUser = {
            ...
            token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjEsInJvbGUiOiJBZG1pbiJ9.ZK7kRVUcGWP7SqLC_rh7xKlVvX2Qj7hQspyOE9Ky',
            role: 'Admin',
            permissions: ['read', 'write', 'admin']
          };
          console.log('Login successful. Auth response:', authUser);
          resolve([200, authUser]);
        } else {
          const errorMessage = "Username and password are invalid. Please enter correct username and password";
          console.log('Login failed:', errorMessage);
          reject([400, errorMessage]);
        }
      });
    });
  });
  ...
  mock.onGet(new RegExp(`${url.GET_PRODUCT_DETAIL}/*`)).reply(config => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (productsData) {
          const { params } = config;
          ...
          if (product) {
            ...
            console.log("Fake product found:", product);
          } else {
